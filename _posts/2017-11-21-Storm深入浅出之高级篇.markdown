...
作为一个示例，我们简单的把信息保存到一个map对象中，在实际场景中你可以把在这里把信息持久化到数据库或Redis缓存中去。另外，你可能还需要考虑Storm的并行性，State的实现最好不要是有状态化的。
...
其中参数S是State的实现类，即`WordCountDB`，而T表示我们从State查询出来的结果类，这里我们使用Long类型。在查询State的时候，对于流中的一批tuple，Trident会调用`batchRetrieve`方法查询得到这一批tuple对应的结果；然后为每个结果调用一次`execute`方法。
...
对于State的更新来说，Trident提供了`StateUpdater`接口，该接口比较简单，不就做过多介绍了。
